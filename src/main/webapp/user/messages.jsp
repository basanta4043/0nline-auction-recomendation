        ...
        <main class="u-full-width ">
            ...
            <c:if test="${not empty messagesLst}">
                <c:forEach var="message" items="${messagesLst}" varStatus="status">
                            ...
                            <div class="message__delete c-delete">
                                ...
                                <form action="${pageContext.request.contextPath}/message.do" method="POST"
                                    ...
                                    <button type="submit" name="action" value="deleteMessage"
                                            class="c-delete__confirm h-fillParent">delete message
                                    </button>
                                </form>
                            </div>
                        </div>
                    ...
                    <c:if test="${user.userId != sendersLst[status.index].userId}">
                        <p class="message message--other"><span
                                class="message__composer">${sendersLst[status.index].firstname} ${sendersLst[status.index].lastname}:</span>
                            <span class="message__text">${message.message}</span> <span
                                    class="message__time">${message.sendDate}</span></p>
                    </c:if>
                </c:forEach>
            </c:if>
            <hr/>
